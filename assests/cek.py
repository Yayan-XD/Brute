...
class Cek_Crack:
    ...
    def hasil(self):
        print(f"""
[{B}1{N}] check hasil crack ok
[{B}2{N}] check hasil crack cp
[{B}3{N}] hapus hasil crack
""")
...
[[bold cyan]00[/]] kembali""", title="[italic bold red]HAPUS HASIL CRACK[/]", padding=(0,5), style="bold white", width=70))
    ...
    def hasil_ok(self):
        ...
        if len(self.hsl_ok)==0:
            exit(f"[{M}x{N}] Tidak ada file di dalam folder {M}results/OK{N} silahkan crack terlebih dahulu")
        else:
